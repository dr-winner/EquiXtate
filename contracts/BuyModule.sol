...
contract BuyModule is Ownable, ReentrancyGuard {
    ...
    function _hashPropertyId(string memory _inputId) internal pure returns (bytes32) {
        bytes memory idBytes = bytes(_inputId);
        require(idBytes.length > 0 && idBytes.length <= MAX_ID_BYTES, "Invalid ID length");
    ...
    function listPropertyForSale(
        ...
        require(
            _duration >= MIN_EXPIRY_DURATION && _duration <= MAX_EXPIRY_DURATION,
            "Invalid expiry duration"
        );

        bytes32 idHash = _hashPropertyId(_idString);
        PropertySale storage existing = sales[idHash];
        require(!existing.isSold && existing.expiresAt < block.timestamp, "Property active or sold");
        ...
        emit PropertyListedForSale(idHash, _idString, _price, _seller, block.timestamp + _duration);
    }
    ...
    function buyProperty(string memory _idString) external nonReentrant {
        ...
        bool success = equiXtate.transferFrom(msg.sender, sale.seller, sale.price);
