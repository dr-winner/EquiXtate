...
contract FractionalOwnershipModule is Ownable {
    ...
    function _validateId(string memory _propertyId) internal pure returns (bytes32) {
        ...
        require(idBytes.length > 0 && idBytes.length <= 64, "Invalid property ID length");
        return keccak256(abi.encodePacked(_propertyId));
    }

    function listProperty(string memory _propertyId, uint256 _valuation, address _propertyOwner) external onlyOwner {
    ...
    function purchaseFraction(string memory _propertyId, uint256 _amount) external {
        bytes32 hashedId = _validateId(_propertyId);
